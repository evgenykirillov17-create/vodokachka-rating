...
function useStorage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const f of FORMATS) {
        try {
          const res = await window.storage.get(`tournaments:${f.id}`, true);
          if (res && res.value) nextT[f.id] = JSON.parse(res.value);
        } catch (e) { /* нет данных ещё */ }
        try {
          const res = await window.storage.get(`cutoff:${f.id}`, true);
          if (res && res.value) nextC[f.id] = JSON.parse(res.value).id;
        } catch (e) { /* нет данных ещё */ }
      }
      try {
        const res = await window.storage.get("roster", true);
        if (res && res.value) nextRoster = JSON.parse(res.value);
      } catch (e) { /* каталога ещё нет */ }
      if (!cancelled) {
  ...
  const saveTournaments = useCallback(async (format, list) => {
    ...
    await window.storage.set(`tournaments:${format}`, JSON.stringify(list), true);
  }, []);

  const saveCutoff = useCallback(async (format, id) => {
    setCutoffs((prev) => ({ ...prev, [format]: id }));
    await window.storage.set(`cutoff:${format}`, JSON.stringify({ id }), true);
  }, []);
  ...
  const addToRoster = useCallback(async (names) => {
    ...
    setRoster(merged);
    await window.storage.set("roster", JSON.stringify(merged), true);
    return merged;
  }, [roster]);
  ...
  const removeFromRoster = useCallback(async (namesToRemove) => {
    ...
    setRoster(filtered);
    await window.storage.set("roster", JSON.stringify(filtered), true);
    return filtered;
  }, [roster]);
  ...
  const restoreAll = useCallback(async (backup) => {
    ...
    for (const f of FORMATS) {
      await window.storage.set(`tournaments:${f.id}`, JSON.stringify(nextT[f.id] || []), true);
      await window.storage.set(`cutoff:${f.id}`, JSON.stringify({ id: nextC[f.id] || null }), true);
    }
    await window.storage.set("roster", JSON.stringify(nextR), true);
    setTournaments(nextT);
    setCutoffs(nextC);
    setRoster(nextR);
  }, []);
